...
def reference(df, region, wave_len, lambd=1e4, smooth_win=25):
    """
            ...
            for mouse in region_to_mouse.mouse.unique():
                for file in os.listdir(path):
                    if '.log' in file and mouse in file:
                        ...
                        logs['Wheel'] = region_to_mouse[region_to_mouse.mouse == mouse].wheel.unique()[0]  # UNCOMMENT WHEN USING WHEEL GROUPS
                        print(region_to_mouse[region_to_mouse.mouse == mouse].wheel)
            
                        dfs.append(logs)
                        print ("Neurophometrics LOGS")
                        print (logs)

    recursive_listdir(Path(project_path), len(subdirs))
    df = pd.concat(dfs)
    df = df.reset_index().set_index([*subdirs, 'Mouse', 'Wheel', 'FrameCounter']) # UNCOMMENT WHEN USING WHEEL GROUPS
...
if __name__ == '__main__':
    ...
    if load_debug_from_disc:
        df = pd.read_csv('../debug_df.csv').set_index(['Group', 'Paradigm', 'Mouse','Wheel', 'Channel', 'FrameCounter'])
        logs = pd.read_csv('../debug_logs.csv').set_index(['Group', 'Paradigm', 'Mouse', 'Wheel','FrameCounter'])
    else:
