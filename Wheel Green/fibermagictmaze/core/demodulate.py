...
PROJECT_PATH = Path(r'/Users/apple/Desktop/Operant YB')
...
def zdFF_airPLS(
    ...
    df, smooth_win=25, remove=100, lambd=5e5, porder=1, itermax=50
):
    """
    ...
    #base_detrend = signal.detrend(s_base)
    #df['s_base'] = s_base
    #df['Detrend'] = base_detrend
    print (df)
    #fig = px.line(df, x='FrameCounter', y=['Signal','s_base'])
    #fig = px.line(df, x='FrameCounter', y=['Detrend', 's_base'])


    #fig.add_line(df, x=df.FrameCounter,y = smooth_signal, line_color='black')
  
    
    
    
    #fig.add_line(df, x=df['FrameCounter'], y=df['s_base'])
    #fig.add_line(df, x=df.FrameCounter,y = smooth_signal, line_color='black')
    #plot(fig)
...
def smooth_signal(x, window_len=25, window="flat"):
    """
...
def airPLS(x, lambda_=25, porder=1, itermax=15):
    """
